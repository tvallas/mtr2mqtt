...
def parse_response(scl_response, scl_response_checksum):
    ...
    if calulated_response_checksum != scl_response_checksum:
        logging.warning(f"Checksum SCL failure, response: {scl_response}, received checksum: {scl_response_checksum} expected checksum: {calulated_response_checksum}")
    try:
        scl_response_content = b''.join(scl_tmp_response_bytes[1:len(scl_tmp_response_bytes)-1]).decode("ascii") 
...
def get_receiver_type(ser,scl_address):
    ...
    try:
        ...
        logging.debug(f"Wrote message: {test_command} to: {ser.name} using settings: {ser.get_settings()}")
        ...
        if response:
            ...
            if response_checksum == expected_checksum:
                ...
                valid_device_type = any([substring in parsed_response for substring in supported_device_types])
                if valid_device_type:
                    return parsed_response
                else:
                    logging.debug(f"Received incorrect device type response: {parsed_response}")
